...
function createStore(reducer$1, preloadedState, enhancer) {
	if (typeof reducer$1 !== "function") throw new Error(`Expected the root reducer to be a function. Instead, received: '${kindOf(reducer$1)}'`);
...
var require_use_sync_external_store_with_selector_development = /* @__PURE__ */ __commonJS({ "node_modules/use-sync-external-store/cjs/use-sync-external-store-with-selector.development.js": ((exports) => {
	(function() {
		...
		var React$1 = require_react(), objectIs = "function" === typeof Object.is ? Object.is : is$1, useSyncExternalStore = React$1.useSyncExternalStore, useRef$1 = React$1.useRef, useEffect$1 = React$1.useEffect, useMemo$1 = React$1.useMemo, useDebugValue = React$1.useDebugValue;
		exports.useSyncExternalStoreWithSelector = function(subscribe, getSnapshot, getServerSnapshot, selector, isEqual$3) {
			...
			instRef = useMemo$1(function() {
				function memoizedSelector(nextSnapshot) {
					if (!hasMemo) {
						...
						if (void 0 !== isEqual$3 && inst.hasValue) {
							var currentSelection = inst.value;
							if (isEqual$3(currentSelection, nextSnapshot)) return memoizedSelection = currentSelection;
						}
					...
					if (void 0 !== isEqual$3 && isEqual$3(currentSelection, nextSelection)) return memoizedSnapshot = nextSnapshot, currentSelection;
					memoizedSnapshot = nextSnapshot;
...
var rafSchd = function rafSchd$1(fn) {
	var lastArgs = [];
	var frameId = null;
...
var getDraggablesInsideDroppable = memoizeOne((droppableId, draggables) => {
	return toDraggableList(draggables).filter((draggable$1) => droppableId === draggable$1.descriptor.droppableId).sort((a, b) => a.descriptor.index - b.descriptor.index);
});
...
var whenReordering = ({ impact, draggable: draggable$1, draggables, droppable: droppable$1, afterCritical }) => {
	...
	if (closestAfter) {
		...
		const withDisplacement = offset(closest$2.page, displacedBy.point);
		return goBefore({
			axis,
...
var getClientFromPageBorderBoxCenter = ({ pageBorderBoxCenter, draggable: draggable$1, viewport }) => {
	...
	const offset$1 = subtract(withoutPageScrollChange, draggable$1.page.borderBox.center);
...
var addPlaceholder = (droppable$1, draggable$1, draggables) => {
	const frame = droppable$1.frame;
	isHomeOf(draggable$1, droppable$1) && invariant(false, "Should not add placeholder space to home list");
	droppable$1.subject.withPlaceholder && invariant(false, "Cannot add placeholder size to a subject when it already has one");
...
var moveToNewDroppable = ({ previousPageBorderBoxCenter, moveRelativeTo, insideDestination, draggable: draggable$1, draggables, destination, viewport, afterCritical }) => {
	if (!moveRelativeTo) {
		...
		const withPlaceholder = isHomeOf(draggable$1, destination) ? destination : addPlaceholder(destination, draggable$1, draggables);
...
var getReorderImpact = ({ pageBorderBoxWithDroppableScroll: targetRect, draggable: draggable$1, destination, insideDestination, last, viewport, afterCritical }) => {
	...
	const closest$2 = removeDraggableFromList(draggable$1, insideDestination).find((child) => {
...
var recomputePlaceholders = ({ draggable: draggable$1, draggables, droppables, previousImpact, impact }) => {
	...
	const patched = addPlaceholder(droppable$1, draggable$1, draggables);
...
var adjustAdditionsForScrollChanges = ({ additions, updatedDroppables, viewport }) => {
	...
	return additions.map((draggable$1) => {
		const droppableId = draggable$1.descriptor.droppableId;
...
function checkIndexes(insideDestination) {
	...
	for (let i = 1; i < indexes.length; i++) {
		...
		if (current !== previous + 1) errors[current] = true;
	}
...
var getNewHomeClientOffset = ({ impact, draggable: draggable$1, dimensions, viewport, afterCritical }) => {
	...
	return subtract(newClientCenter, draggable$1.client.borderBox.center);
};
...
function getScrollListener({ onWindowScroll }) {
	...
	function start$1() {
		isActive$1() && invariant(false, "Cannot start scroll listener when already active");
		unbind = bindEvents(window, [binding]);
	}
	function stop() {
		!isActive$1() && invariant(false, "Cannot stop scroll listener when not active");
		scheduled.cancel();
		unbind();
		unbind = noop$2;
	}
	...
	const collect = () => {
		...
		frameId = requestAnimationFrame(() => {
			...
			const updated = Object.keys(modified).map((id) => {
				const scroll$2 = registry.droppable.getById(id).callbacks.getScrollWhileDragging();
				return {
					droppableId: id,
					scroll: scroll$2
				};
...
var getScrollableDroppableOver = (target, droppables) => {
	return getScrollableDroppables(droppables).find((droppable$1) => {
		!droppable$1.frame && invariant(false, "Invalid result");
...
var createJumpScroller = ({ move: move$1, scrollDroppable: scrollDroppable$1, scrollWindow: scrollWindow$1 }) => {
	const moveByOffset = (state, offset$1) => {
		const client = add(state.current.client.selection, offset$1);
		move$1({ client });
	};
...
function useUniqueId(prefix$3, options = defaults) {
	...
	return useMemo(() => `${prefix$3}${options.separator}${id}`, [
		options.separator,
		prefix$3,
		id
	]);
...
function useValidation$1(props, contextId, getRef) {
	useDevSetupWarning(() => {
		function prefix$3(id$1) {
			return `Draggable[id: ${id$1}]: `;
		}
      ...
      Provided: [type: ${typeof id}] (value: ${id})`);
		!Number.isInteger(props.index) && invariant(false, `${prefix$3(id)} requires an integer index prop`);
		if (props.mapped.type === "DRAGGING") return;
		checkIsValidInnerRef(getRef());
		if (props.isEnabled) !findDragHandle(contextId, id) && invariant(false, `${prefix$3(id)} Unable to find drag handle`);
	});
}
...
function getSecondarySelector() {
	...
	const getProps = (ownId, draggingId, impact, afterCritical) => {
		...
		if (!visualDisplacement) {
			...
			const offset$2 = memoizedOffset(change.x, change.y);
			return getMemoizedProps(offset$2, combineTargetFor, true);
		}
		if (isAfterCriticalInVirtualList) return getFallback(combineTargetFor);
		const displaceBy = impact.displacedBy.point;
		const offset$1 = memoizedOffset(displaceBy.x, displaceBy.y);
		return getMemoizedProps(offset$1, combineTargetFor, visualDisplacement.shouldAnimate);
	};
	const selector = (state, ownProps) => {
		if (isDragging(state)) {
			if (state.critical.draggable.id === ownProps.draggableId) return null;
...
var isBodyScrollable = () => {
	...
	if (isBoth(htmlOverflow, isVisible)) return false;
	warning(`
...
var getEnv = (start$1) => {
	const closestScrollable = getClosestScrollable(start$1);
	const isFixedOnPage = getIsFixed(start$1);
	return {
		closestScrollable,
		isFixedOnPage
	};
...
function useDroppablePublisher(args) {
	...
	const onClosestScroll = useCallback(() => {
		...
		!(dragging && closest$2) && invariant(false, "Could not find scroll options while scrolling");
	...
	const getScrollWhileDragging = useCallback(() => {
		const dragging = whileDraggingRef.current;
		const closest$2 = getClosestScrollableFromDrag(dragging);
		!(dragging && closest$2) && invariant(false, "Can only recollect Droppable client for Droppables that have a scroll container");
	...
	const scroll$2 = useCallback((change) => {
		...
		const closest$2 = getClosestScrollableFromDrag(dragging);
		!closest$2 && invariant(false, "Cannot scroll a droppable with no closest scrollable");
