...
import { Provider_default, applyMiddleware, bindActionCreators, compose, connect_default, createStore, invariant as invariant$1 } from "./tiny-invariant-Blx7gPte.js";
...
function useUniqueId(prefix$2, options = defaults) {
	const id = import_react.useId();
	return useMemo(() => `${prefix$2}${options.separator}${id}`, [
		options.separator,
		prefix$2,
		id
	]);
...
function useValidation$1(props, contextId, getRef) {
	useDevSetupWarning(() => {
      ...
      Provided: [type: ${typeof id}] (value: ${id})`);
		!Number.isInteger(props.index) && invariant(false, `${prefix$2(id)} requires an integer index prop`);
		if (props.mapped.type === "DRAGGING") return;
		checkIsValidInnerRef(getRef());
		if (props.isEnabled) !findDragHandle(contextId, id) && invariant(false, `${prefix$2(id)} Unable to find drag handle`);
	});
}
