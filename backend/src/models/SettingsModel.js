...
export async function upsertSettingsModel({
  ...
  // ✅ loyalty dynamic conversions (with safe defaults)
  const pointsPerGbp =
  ...
  const availableHours =
    loyalty_available_after_hours !== undefined && loyalty_available_after_hours !== ""
  ...
  if (existing.length > 0) {
    ...
    const [result] = await db.query(
      `INSERT INTO settings
       (signup_flat_amount, referral_flat_amount, minimum_order,
        loyalty_points_per_gbp, loyalty_redeem_points, loyalty_redeem_value,
        loyalty_available_after_hours, loyalty_expiry_days)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        signup,
        referral,
